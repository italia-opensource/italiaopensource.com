...
function getRepositoryHTML(data: any) {
    // autogenerated.analytics/meta are populated monthly by awesome-italia-opensource;
    // recently-added repos can arrive without them, so fall back to the tags-only view
    // instead of crashing the static build.
    const hasAnalytics =
        data.autogenerated?.analytics && data.autogenerated?.meta;

    switch (data.repository_platform) {
        case 'github':
            return hasAnalytics
                ? getRepositoryGitHub(data)
                : getRepositoryGitLab(data);
        default:
